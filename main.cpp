...
#define SOIL_SENSOR_1 0    // GPIpinO 17 (wiringPi pin 0)
#define SOIL_SENSOR_2 2    // GPIO 27 (wiringPi pin 2)
#define PUMP1_RELAY 21     // GPIO 5 (wiringPi pin 21)
#define PUMP2_RELAY 22     // GPIpinO 6 (wiringPi pin 22)
...
// Servo PWM settings
#define SERVO_PWM_CLOCK 384         // Clock divisor
#define SERVO_PWM_RANGE 1000        // Range for finer control
#define SERVO_CENTER 75             // 1.5ms pulse (75/1000 * 20ms = 1.5ms)
#define SERVO_LEFT 50               // 1.0ms pulse (left position)
#define SERVO_RIGHT 100             // 2.0ms pulse (right position)   
...
public:
    ...
    void setServoPosition(int position) {
        ...
        if (position == -1) {
            ...
            pwmValue = SERVO_RIGHT; // Right position
        } else {
            pwmValue = SERVO_CENTER; // Center position
        }
    ...
    void run() {
        ...
        logMessage("Servo PWM config: Clock=" + 
            std::to_string(SERVO_PWM_CLOCK) + ", Range=" + std::to_string(SERVO_PWM_RANGE));
        if (lcdEnabled) {
